...
summary lives in `AGENTS.md` (which is loaded into your AI
assistant's context by default); read **this** file when you need detail.

## At a glance

- **Project URL:** `https://aip.softcat.io/alex/dfe-digital-standards-copilot`
- **Owner:** `alex`
...
- This project is owned by a single user within a single Softcat-customer
  ...
  portal at publish time): `private` (only the owner — useful for tools
  you're iterating on), `tenant` (any member of the owner's org), or
...
The Publish step clones this repo into a fresh sandbox and runs the
configured build command (`pnpm install --frozen-lockfile=false && pnpm build`). If
that exits non-zero, the publish fails with `build exited with code
<N>` and the site stays on the previous successful build.

### Don't shadow DOM globals with domain types

TypeScript binds names in lexical scope first. If you declare a domain
type called `Response` — common when modelling assessment answers,
HTTP-like records, or API responses — every
`fetch(...).then((res: Response) => res.json())` callback in this repo
binds to **your** type, not the DOM `Response` from
`lib.dom.d.ts`. The build then rejects with cascades of:

```
src/api.ts(6,12): error TS2339: Property 'ok' does not exist on type 'Response'.
src/api.ts(9,27): error TS2339: Property 'json' does not exist on type 'Response'.
```

The same trap exists for `Request`, `Headers`, `URL`, `Blob`,
`File`, `FormData`, `Event`, `Location`, `Document`, `Window`.
Prefix domain types that would collide: `AssessmentResponse`,
`ApiRequest`, `EventRecord`, `SiteLocation`, etc.

### How to catch it locally

Run `pnpm install --frozen-lockfile=false && pnpm build` before pushing. The publish
saga runs the same command in CI, so anything that breaks locally will
break the publish.
